...
class Mongo_db {
	...
	*/
	...
	public function count($collection = "", $is_clear = true) {
		if (empty($collection))
		{
	...
	*/
	...
	private function connection_string() 
	{
		...
		$this->replname = trim($this->CI->config->item('mongo_replname'));
		$dbhostflag = (bool)$this->CI->config->item('host_db_flag');
		$replname_flag = (bool)$this->CI->config->item('host_replname_flag');
		$host_2_flag = (bool)$this->CI->config->item('mongo_host_2_flag');
		$this->host_2 = trim($this->CI->config->item('mongo_host_2'));
        ...
        if ($host_2_flag === TRUE)
        {
            ...
            {
                $connection_string .= ",";
                $connection_string .= "{$this->host_2}:{$this->port}";
            }
            else
            {
                $connection_string .= "{$this->host_2}";
            }
        }
		...
		if ($replname_flag === TRUE)
		{
			$this->connection_string = trim($connection_string) . '?replicaSet=' . $this->replname ;
		}		
		else
		{
